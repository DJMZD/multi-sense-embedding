...
BOOST_AUTO_TEST_CASE(vocab_serialization_test) {
  string punctuation = "/⁄\\()\"':,.;<>~!@#$%^&*|+=[]{}`?-…-‹›«»‐-‘’“”";
  auto text_splitter = boost::make_shared<RawtextSplitter>(punctuation);
  Vocab vocab("data/test", 2000, text_splitter);
  ofstream ofs("data/vocab-serialize.test");
  boost::archive::text_oarchive oa(ofs);
  oa << vocab;
  ofs.close();
  ifstream ifs("data/vocab-serialize.test");
